...
一覧に `[投稿結果不明]` と出たジョブは、送信後に応答または公開確認だけが失敗した可能性がある。
対象SNSを確認し、未投稿なら前者、投稿済みなら後者を実行する。

    ruby bin/retry --confirm-not-posted failed\対象ジョブ.json
    ruby bin/retry --confirm-posted failed\対象ジョブ.json

オプションなしの `retry` は結果不明ジョブを拒否するため、確認せず二重投稿することはない。
...
- 確認済み候補の清掃: `ruby bin\cleanup --apply --days 30`（Chromeキャッシュは除外）
- Chromeキャッシュも清掃: 専用Chromeをすべて閉じ、上記へ `--include-browser-cache` を追加
...
- `bin/watch` は最新40件までを対象とする。未取得が40件を超えた場合は古い投稿を取り込まず、全履歴同期のためのページングは行わない
