...
- Threadsはテキスト、単画像、複数画像カルーセルをAPIで実投稿確認済み
...
| Threads | 実装済み・予備経路 | 公式API。通常運用は画像付きをInstagram、テキストだけをFacebookから波及。APIはテキスト・単画像・複数画像を実投稿確認済み |
...
`watch` は「最近の投稿をほかのSNSへ展開する」用途とし、1回につき最新40件までを取得する。
停止中などに未取得投稿が40件を超えた場合は、新しい40件を対象にして、それより古い投稿は
取り込まない。全履歴の欠落なき同期は目的とせず、ページングは実装しない。
...
明示的なテスト用として維持する。2026-09-06にはFedibirdの静止画像2枚からカルーセル投稿し、
Threads上で1件の投稿に2枚表示されることを実地確認した。テスト投稿は確認後に削除した。
