...
- 日付: 2026-08-06
- 状態: テキストは実投稿確認完了（2026-08-08）。画像は実装・自動テスト完了、実投稿確認待ち（2026-08-11）
- 対象入口: `targets.post` と `targets.watch`。画像はFedibird監視ジョブの `media_urls` を利用
...
Fedibird監視時にジョブへ保存済みの元画像URLを使い、画像1枚ではIMAGEコンテナを作成、2～20枚では
...
カルーセル作成・処理待ち・公開まで確認済み。実アカウントでの投稿確認後に状態を更新する。
