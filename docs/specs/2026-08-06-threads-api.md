...
- 日付: 2026-08-06
- 状態: テキスト、単画像、複数画像カルーセルの実投稿確認完了（2026-09-06）
...
カルーセル作成・処理待ち・公開まで確認済み。2026-08-11にはFedibird最新投稿の画像1枚を使い、
ThreadsとBloggerへ `bin/post --from-fedibird-latest` → `bin/run_queue` で投稿した。両方の公開表示を
確認し、テスト投稿は確認後に削除した。実際の投稿IDや認証情報は保存しない。

2026-09-06にはFedibird最新投稿の静止画像2枚を使い、同じ手順でThreads APIへ投稿した。
Threads上で1件の投稿に2枚の画像が表示されることを確認し、Instagramからの通常波及による
投稿と重複したため、API側のテスト投稿は確認後に削除した。

通常運用は、画像付きをInstagram、テキストだけをFacebookからThreadsへ波及させる。
API投稿は予備・テスト経路として維持する。Threads固有のテキスト・単画像・複数画像の
実地確認は完了した。
