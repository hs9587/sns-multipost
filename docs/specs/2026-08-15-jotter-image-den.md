...
2026-08-19の画像投稿では、ウォレット確認後に投稿フォームが開いたものの、本文入力前の画面更新で
`NodeNotFoundError` になった。失敗画面は本文・画像とも未入力で、送信前と確認できた。本文入力、
画像選択、確認Okでは、取得済みDOM要素が消えた場合に要素を取り直して最大3回まで再試行する。

2026-09-09には、前日の同文投稿URLを新規投稿と誤認した可能性がある事象を確認した。投稿前には
URLが `/ja-JP/` でハッシュのないホームへ遷移済みであることを必須とし、遷移前の個別投稿URLも
既存URLへ加える。投稿後は投稿前に存在しなかったURL、本文、個別画面に加え、画面内の `time`
要素または日本語の公開日時が投稿開始10分前以降であることを確認する。日時を確認できない場合は
成功扱いせず、投稿結果不明として人が確認する。

2026-08-23には本文・画像付き個別投稿が表示済みでも、画面パスが `/jot/` と一致せず確認だけ失敗した。
個別投稿は `/jot/`、`/doc/`、ハッシュ経路のいずれでも、今回の本文表示を確認できれば成功とする。
