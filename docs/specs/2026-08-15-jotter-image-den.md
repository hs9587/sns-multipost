...
Post後はJavaScript標準ダイアログを自動承認し、HTML上の `Ok` ボタンが現れた場合も押す。
成功判定は、画面が新しい `/ja-JP/jot/#...` の個別投稿へ移った場合、またはホームに今回の本文を
含む新規投稿リンクが現れた場合とする。連続する複数のHTML確認ダイアログを段階別に識別することと、
画像投稿では個別投稿画面への遷移を優先確認することは、今後の安定化課題とする。
