...
- 日付: 2026-09-06
- 状態: dry-run・実清掃実装、自動テスト・初回実データ清掃完了
...
失敗した項目は表示して終了コード1を返す。実データを清掃するときは、運用者が直前のdry-run結果を
再確認してから行う。

## 初回実清掃

2026-09-07に30日基準で再確認し、古い完了ジョブ69件、未参照の古い画像39ディレクトリ、
再生成可能なChromeキャッシュ40ディレクトリ、合計148件・1530.44 MiBを削除した。削除失敗は
0件。直後のdry-runでは全分類が0件となり、failed JSON 14件と参照中media 7ディレクトリは
保護されたままであることを確認した。今後は増加傾向を見て、30日の保持期間を継続評価する。
