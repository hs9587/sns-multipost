...
- ファイル選択後はGoogle画像追加フレームのファイル入力が画像を受理したことを確認し、
  アップロード処理の静定時間を置いてから、使用可能になった「選択／挿入」ボタンを押す。
  Google側の変動するプレビューURLや内部DOMは完了条件にせず、最後にBlogger本文への
  画像挿入を確認する。
- 「選択／挿入」のクリックによって画像追加フレームが閉じ、CDPの応答前に実行コンテキストが
  破棄された場合は、クリック失敗とはせずBlogger本文側の画像出現を待つ。閉じたフレームが
  Chromeのフレーム一覧に一時的に残り、URLや実行コンテキストを読めない場合も確認対象から除外する。
